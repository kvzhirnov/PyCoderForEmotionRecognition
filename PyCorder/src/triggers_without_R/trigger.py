...
class TRG_Eeg(ModuleBase):
    ...
    def searchTrigger(self, in_out):
        ...
        if in_out == 0: 
            trigger = np.bitwise_and(self.data.trigger_channel[0], 0x00FF) # mask trigger input channels 0-7
        elif in_out == 1:
            trigger = np.bitwise_and(self.data.trigger_channel[0], 0x0000) # no R triggers markers
        else:
